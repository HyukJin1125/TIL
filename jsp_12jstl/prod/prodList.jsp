...
<body>
	<%
	List<ProdVO> prdList = ProductList.getProductList();
	...
	<div class="container">
		...
		<table class="prod-list">
			...
			<tbody>
				<c:forEach items="${prdList}" var="prod" varStatus="varStat">
					<c:if test="${varStat.count%3 == 1}"><tr></c:if>
					<td>
						<ul>
							<li class="prod-image"><a href="prodView.jsp?prodId=${prod.prodId}"><img
									alt="" src="/study${prod.prodImg}"></a>
					...
					<c:if test="${varStat.count%3 == 0}"></tr></c:if>
				</c:forEach>
			</tbody>
