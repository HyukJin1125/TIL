...
public class PlayMap {
	...
	public static void main(String[] args) {
		...
		/*
		 * 노래제목은 key, 가수명은 value로 하는 HashMap을 만들어 playList의 데이터들을 담아보세요.
		 */
		HashMap<String, String> playMap = new HashMap<>();
		for (String music : playList) {
			String song = music.split(": ")[0];
			String art = music.split(": ")[1];
			playMap.put(song, art);
		}
	...
	static HashMap<String, String> searchSong(String option, String keyWord, HashMap<String, String> inputMap) {
		...
		for(String key : keySet){
			if (option.equals("노래")) {
				...
				String singger = inputMap.get(key);
				if(singger.contains(keyWord)) {
					resultMap.put(key, inputMap.get(key));
				}
			}
		}
